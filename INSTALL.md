...
DATABASE_URL=Railway PostgreSQL 连接地址
DJANGO_SUPERUSER_USERNAME=后台管理员账号
DJANGO_SUPERUSER_PASSWORD=后台管理员密码
DJANGO_SUPERUSER_EMAIL=后台管理员邮箱
DJANGO_SUPERUSER_NICKNAME=后台管理员昵称
ALIPAY_ENV=sandbox
...
课程详情页的“支付宝扫码支付”使用当面付 `alipay.trade.precreate` 生成二维码，沙箱测试时必须用支付宝沙箱 App 扫码。
